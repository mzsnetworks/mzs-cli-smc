# Master — Your network diagram is a liability

**Idea:** Documentation that describes intent instead of reality actively misleads the 2am troubleshooter. Docs must describe what IS, not what was planned.
**Pillar:** Engineering Systems · **Format:** Contrarian
**Status:** fact-checked (no external stats — experience post)
...
The fix is documentation that can't drift: generated from the network, not about the network. Pull it from the running configs. Reconcile your source of truth against live state on a schedule. If a diagram matters, generate it — a document produced by the network is evidence; a document produced about the network is a memory.
