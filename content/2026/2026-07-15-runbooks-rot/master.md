# Runbooks Rot When Nobody Runs Them

**Thesis:** A runbook is only true the day it's written; the network keeps moving and the doc goes stale silently. The only runbook you can trust is one that gets exercised — automated (the script is the runbook) or rehearsed (game days).
**Pillars:** Engineering Systems · Lessons from Production · Enterprise Network Operations
**Defended opinion:** The value of a runbook was never in writing it — it's in running it. An unexecuted procedure is an untested assumption in a nice font.
