# Master — slow-is-not-a-symptom

**Preset:** Professional (LinkedIn personal profile + Instagram @mzsnetworks)
**Date:** 2026-10-20
**Thesis:** "It's slow" is not a symptom, it is a feeling. The first job is converting it into a number with a direction and a boundary, because you cannot tell whether you have fixed a feeling, and an unmeasured complaint can never be closed.
**Pillar:** Enterprise Network Ops
**Source:** ideas-2026-08-27 #6
...
A direction. Is it slow in one direction, both, or only on the first request? Upload and download failing differently is a completely different problem from both failing together, and it is the single cheapest split available.
...
Then one more question that is not technical: when did this become worth reporting? Sometimes the answer is that nothing changed in the network and something changed in what the business expects of it. That is still a real problem. It is just not one you fix with a packet capture.

None of this is the investigation. It is the thing that makes an investigation possible, and it takes about ten minutes.
