# Infrastructure-as-Wishes

**Thesis:** A repo is the source of truth only if the network can't diverge from it without being caught and corrected. Without drift detection + enforcement, IaC is a confident description of a network that no longer exists — infrastructure-as-wishes.
**Pillars:** Network Automation · Engineering Systems · Brownfield Engineering
**Defended opinion:** The repo being version-controlled was never the point. The repo being true is the point — and enforcement is what makes it true.
...
Real infrastructure-as-code needs two things most "IaC" setups skip: detection that continuously compares intended state to actual state, and enforcement that either reverts the drift or forces it back through the repo. No out-of-band path that sticks. If someone can change the network and the repo doesn't notice, the repo is documentation — and documentation drifts.
