# LinkedIn — documentation-that-executes
...
The practical shape is unglamorous. Tag at the edge, where you still know what something is: which customer, which region, which relationship, which class of route. Then write policy that matches on the tag rather than on a prefix list somebody maintains by hand. A prefix list is a wiki that happens to be machine-readable, and it goes stale the same way.

Two rules keep it honest.

A community nobody matches on is a comment, not documentation, and it rots like one. If you are tagging routes and no policy reads the tag, you have built an elaborate way of writing notes to yourself.

And publish the scheme. A community whose meaning lives in one engineer's head is worse than no community, because the next person infers a meaning from what it appears to do, and they will be subtly wrong.

Where does your routing intent actually live: in something the router reads, or in something only people read?

#NetworkEngineering #BGP #NetOps
